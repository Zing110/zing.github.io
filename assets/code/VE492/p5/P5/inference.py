...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        total = self.total()
        if total == 0:
            raise ValueError("The distribution is empty or all values are zero.")
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i, noisyDist in enumerate(observation):
                ...
                weight *= self.getObservationProb(noisyDist, pacmanPos, ghostPos, jailPos)
            updatedDistribution[particle] += weight

        if updatedDistribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [updatedDistribution.sample() for _ in range(self.numParticles)]
