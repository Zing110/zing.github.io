    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # Compute the most likely position of each living ghost
        ...
        closestGhostPos = mostLikelyPositions[ghostDistances.index(min(ghostDistances))]

        # Choose an action that brings Pacman closest to the closest ghost
        successorDistances = [self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestGhostPos) for action in legal]
